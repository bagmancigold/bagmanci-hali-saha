...
export default function AdminPage() {
  ...
  if (!loggedIn) return <main className="flex min-h-screen items-center justify-center bg-[var(--green)] px-5 py-12"><div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl sm:p-10"><a href="/" className="mb-10 flex items-center gap-2 text-sm font-bold text-[var(--green)]"><ArrowLeft size={16} /> Siteye dön</a><div className="mb-8 flex h-14 w-14 items-center justify-center rounded-2xl bg-[var(--lime)] text-[var(--green)]"><LockKeyhole size={26} /></div><p className="text-xs font-bold uppercase tracking-[.18em] text-[var(--green)]">Bağmancı Halı Saha</p><h1 className="display mt-3 text-4xl font-extrabold">Admin girişi</h1><p className="mt-3 text-sm leading-6 text-[var(--muted)]">Rezervasyonları ve saha ayarlarını yönetmek için giriş yap.</p><label className="mt-8 block text-sm font-bold">Kullanıcı adı<input value={loginForm.username} onChange={(event) => setLoginForm({ ...loginForm, username: event.target.value })} className="mt-2 w-full rounded-xl border border-[var(--line)] px-4 py-3 outline-none focus:border-[var(--green)]" placeholder="admin" /></label><label className="mt-4 block text-sm font-bold">Şifre<input type="password" value={loginForm.password} onChange={(event) => setLoginForm({ ...loginForm, password: event.target.value })} className="mt-2 w-full rounded-xl border border-[var(--line)] px-4 py-3 outline-none focus:border-[var(--green)]" placeholder="Şifreniz" /></label><button onClick={handleLogin} className="mt-6 w-full rounded-full bg-[var(--green)] px-5 py-4 text-sm font-extrabold text-white">Giriş yap</button>{loginError && <p className="mt-4 rounded-xl bg-red-50 p-3 text-sm font-semibold text-red-700">{loginError}</p>}<p className="mt-6 text-xs leading-5 text-[var(--muted)]">Bu panel arayüz demosudur. Gerçek yayında sunucu tarafı kimlik doğrulama eklenmelidir.</p></div></main>;
