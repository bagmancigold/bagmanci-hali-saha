...
export default function AdminPaymentsPage() {
  ...
  const load = async () => {
    try {
      ...
      setAuthorized(true);
      let [{ data: bookings, error }, { data: settings }] = await Promise.all([
      ...
      if (error?.message.includes("paid_amount")) {
        const fallback = await client.from("booking_requests").select("id, customer_name, phone, booking_date, booking_time, package_name, total_amount, payment_status").order("booking_date", { ascending: false }).order("booking_time", { ascending: true });
        bookings = (fallback.data || []).map((item) => ({ ...item, paid_amount: 0 }));
        error = fallback.error;
        if (!error) setMessage("Ödenen tutar alanı Supabase şemasında yok. Güncel supabase-schema.sql dosyasını çalıştırın.");
      }
    ...
    } catch (error) { setAuthorized(false); setSelectedDate(today); setMessage(error instanceof Error ? error.message : "Ödemeler yüklenemedi."); }
  };
  ...
  const formatDate = (date: string) => { const parsed = new Date(`${date || today}T12:00:00`); return new Intl.DateTimeFormat("tr-TR", { weekday: "long", day: "numeric", month: "long" }).format(Number.isNaN(parsed.getTime()) ? new Date() : parsed); };
