...
export default function AdminPaymentsPage() {
  ...
  const savePayment = async (item: Booking) => { const paid = Number(amounts[item.id] || 0); const status = paid <= 0 ? "unpaid" : paid >= Number(item.total_amount) ? "paid" : "deposit"; const { error } = await getSupabaseClient().from("booking_requests").update({ paid_amount: paid, payment_status: status }).eq("id", item.id); if (error) setMessage(error.message); else { setItems((current) => current.map((row) => row.id === item.id ? { ...row, paid_amount: paid, payment_status: status } : row)); setMessage(`Durum güncellendi: ${labels[status]}.`); } };
