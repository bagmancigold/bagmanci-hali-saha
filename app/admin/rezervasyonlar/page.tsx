...
export default function AdminBookingsPage() {
  ...
  return <main className="reservation-page min-h-screen bg-[#f5f7f3] px-3 py-6 text-[var(--ink)] sm:px-5 lg:px-8"><div className="mx-auto max-w-[1600px]"><div className="mb-5 flex items-center justify-between"><a href="/admin" className="inline-flex items-center gap-2 text-xs font-bold text-[var(--green)]"><ArrowLeft size={15} /> Admin paneline dön</a><div className="flex gap-1.5"><button type="button" onClick={() => setWeekOffset((value) => value - 1)} className="rounded-full border bg-white p-2" aria-label="Önceki hafta"><ChevronLeft size={16} /></button><button type="button" onClick={() => setWeekOffset((value) => value + 1)} className="rounded-full border bg-white p-2" aria-label="Sonraki hafta"><ChevronRight size={16} /></button><button type="button" onClick={load} className="rounded-full border bg-white p-2" aria-label="Yenile"><RefreshCw size={16} /></button></div></div><div className="mb-4 flex items-end justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-[var(--green)]">Haftalık rezervasyon defteri</p><h1 className="display mt-1 text-2xl font-extrabold sm:text-3xl">Rezervasyonlar</h1></div><p className="text-right text-xs text-[var(--muted)]">{dateText(weekStart)} - {dateText(new Date(weekStart.getTime() + 6 * 86400000))}</p></div>{message && <p className="mb-3 rounded-lg bg-white p-3 text-xs font-semibold text-[var(--green)]">{message}</p>}<div className="reservation-scroll overflow-x-auto rounded-xl border border-[var(--line)] bg-white shadow-sm"><div className="reservation-grid min-w-[1280px]"><div className="reservation-corner">Gün / Saat</div>{hours.map((hour) => <div key={hour} className="reservation-hour">{hour}</div>)}{dates.map((date, dayIndex) => <div className="contents" key={date}><div className="reservation-day"><strong>{days[dayIndex]}</strong><span>{date.slice(8, 10)}.{date.slice(5, 7)}</span></div>{hours.map((hour) => { const booking = bookingAt(date, hour); return <div key={`${date}-${hour}`} className={`reservation-cell ${booking ? "reservation-cell-booked" : ""}`}>{booking && <><strong>{booking.customer_name}</strong><span>{booking.phone}</span><b>₺{booking.total_amount}</b><em>{statusLabels[booking.payment_status] || booking.payment_status}</em></>}</div>; })}</div>)}</div></div><p className="mt-3 text-[11px] text-[var(--muted)]">Dolu saatlerde takım kaptanı, telefon, ücret ve ödeme durumu görünür. Uzun rezervasyonlar kapladığı tüm saatlerde işaretlenir.</p></div></main>;
}
